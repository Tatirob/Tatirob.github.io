...
if( !class_exists( 'UT_Media_Slider' ) ) {
	...
	class UT_Media_Slider {
                    ...
                    $(document).ready(function(){
                        ...
                        if ( $().lightGallery ) {
                            ...
                            $owl_<?php echo esc_attr( $id ); ?>.on( 'changed.owl.carousel', function ( event ) {
                                ...
                                $lightgallery_<?php echo esc_attr( $id ); ?>.data( 'lightGallery' ).destroy( true, true );
                                $lightgallery_<?php echo esc_attr( $id ); ?>.lightGallery( lightgallery_<?php echo esc_attr( $id ); ?> );
		...
		function create_image_slide( $image, $caption, $atts ) {
			...
			$lightgallery = wp_get_attachment_image_src( $image_id, 'ut-lightbox' );
			...
			if( !empty( get_post( $image_id )->post_excerpt ) ) {
				...
				return '<div><figure data-caption="' . esc_attr( get_post( $image_id )->post_excerpt ) . '" class="ut-gallery-slider-caption-wrap">' . $link_icon . '<img class="item owl-lazy" alt="' . esc_attr( get_post($image_id)->post_excerpt ) . '" data-src="' . esc_url( $thumbnail[0] ) .'" /></figure>' . $caption . '<div id="ut-image-caption-' . $image_id . '" class="ut-vc-images-lightbox-caption">' . get_post( $image_id)->post_excerpt . '</div></div>';
		...
		function create_video_slide( $video, $caption, $atts ) {
		...
		function ut_create_shortcode( $atts, $content = NULL ) {
			...
			if( !empty( $slides ) && is_array( $slides ) ) {
				...
				foreach( $slides as $slide ) {
					...
					if( $slide['type'] == 'image' ) {
						$output .= $this->create_image_slide( $slide, $caption, $atts );
					}

					if( $slide['type'] == 'video' ) {
						$output .= $this->create_video_slide( $slide, $caption, $atts );
					}
