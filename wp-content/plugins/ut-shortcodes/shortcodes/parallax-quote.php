...
if ( class_exists( 'WPBakeryShortCode' ) ) {
    ...
    class WPBakeryShortCode_ut_parallax_quote extends WPBakeryShortCode {
        ...
        function ut_create_inline_script( $id, $atts ) {
            ...
            <script type="text/javascript">
                ...
                (function ($) {
                    ...
                    $(document).ready( function () {
                        ...
                        if( $('#<?php echo $id; ?>').prev('.ut-parallax-icon-wrap').length ) {
                           
                            var <?php echo $id; ?>_icon = new RevealFx( $('#<?php echo $id; ?>').prev('.ut-parallax-icon-wrap')[0], {
                                revealSettings : {
                                    bgcolor: '<?php echo $revealfx_color; ?>',
                        ...
                        var <?php echo $id; ?> = new RevealFx(document.querySelector('#<?php echo $id; ?>'), {
                            revealSettings : {
                                bgcolor: '<?php echo $revealfx_color; ?>',
                                onCover: function(contentEl, revealerEl) {
                                    contentEl.style.opacity = 1;
                        ...
                        if( $('#<?php echo $id; ?> + .ut-parallax-quote-name-wrap').length ) {
                           
                            var <?php echo $id; ?>_name = new RevealFx( $('#<?php echo $id; ?> + .ut-parallax-quote-name-wrap')[0], {
                                revealSettings : {
                                    bgcolor: '<?php echo $revealfx_color; ?>',
                                    onCover: function(contentEl, revealerEl) {
                                        contentEl.style.opacity = 1;
                        ...
                        $(document.body).on('appear', '#<?php echo $id; ?>', function() {
                            ...
                            // add done class
                            $this.addClass('ut-block-reveal-done');                            
            ...
            // hook footer script
            add_action( 'wp_footer', array( $this, 'revealfx_enqueue_scripts' ) );
        ...
        function revealfx_enqueue_scripts() {
            
            if( !$this->add_script ) {
                return;
            }
            
            wp_print_scripts('ut-revealfx-js');
        ...
        protected function content( $atts, $content = null ) {
			...
			if( $revealfx == 'on' ) {
                ...
                $output .= $this->ut_create_inline_script( $reveal_id, $atts );
                
                // extra classes
                $reveal_classes[] = 'ut-block-reveal-hide';
                $reveal_wrap_classes[] = 'ut-element-with-block-reveal';
            ...
            $output .= '<div id="' . $id . '" class="ut-parallax-quote ' . implode( " ", $reveal_wrap_classes ) . '">';
                
                if( $icon ) {
                    
                    $output .= '<div class="ut-parallax-icon-wrap">';
                ...
                $output .= '<blockquote data-appear-top-offset="-200" id="' . esc_attr( $reveal_id ) . '" class="ut-parallax-quote-title ' . implode( " ", $reveal_classes ) . ' element-with-custom-line-height ' . esc_attr( $class ) . '" ' . $quote_inline_styles . '>';
